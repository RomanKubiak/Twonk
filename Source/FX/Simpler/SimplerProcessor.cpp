...
SimplerProcessor::SimplerProcessor(const PluginDescription &descr)
    ...
    processorState.createAndAddParameter(
            std::make_unique<AudioParameterInt>(
                    "bank",
                    "bank",
                    0,
                    1, //GET_TWONK_SAMPLES_DIR().getNumberOfChildFiles(File::findDirectories, "*.twonk"),
                    0,
                    "bank",
                    bankIndexToName,
                    bankNameToIndex));

    processorState.createAndAddParameter("ampEnvAttack",
                                         "ampEnvAttack",
                                         "ENV/Attack",
                                         NormalisableRange<float>(0.0f, 500.0f),
                                         0.0f,
                                         doubleToString,
                                         stringToDouble);
...
void SimplerProcessor::loadBank(const File &bank) {
    ...
    if (bank.getChildFile("info.json").existsAsFile()) {
        ...
        if (parsingResult.wasOk()) {
            ...
            AlertWindow::showMessageBox(AlertWindow::WarningIcon, "Bank creation failure",
                                        "Can't create a valid json description for directory: " + bank.getFileName());
        }
    }
}

...
struct PizNote
{
	...
	{
		object = _object;
		jassert(_object->noteOffObject.get()!=0);
		length = _object->noteOffObject.get()!=0 ? _object->noteOffObject->message.getTimeStamp() - _object->message.getTimeStamp() : 0;
	}
	void updateLength() {
		if (object->message.isNoteOn()) {
			jassert(object->noteOffObject.get()!=0);
			length = object->noteOffObject.get()!=0 ? object->noteOffObject->message.getTimeStamp() - object->message.getTimeStamp() : 0;
		}
	}
...
public:
	...
	{
		this->getParentComponent()->mouseWheelMove(event, wheel);
	}
